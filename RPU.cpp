 ...
 *     This file is part of the RPU for Arduino Project.
    ...
    - Revised to allow RPU_SetLampState to control strobe lamps (e.g., Flash Gordon)
...
/******************************************************
 *   Variables to control strobe lamps
      RPUstrobeLamp: Lamp number
      RPUstrobeType: 0 = Strobe signal always off
 ...
 */
byte RPUstrobeType =  0;
int  RPUstrobeLamp = 99;
...
void RPU_SetDisplayFlashCredits(unsigned long curTime, int period) {
  if (period) {
    if ((curTime/period)%2) {
      DisplayDigitEnable[4] |= 12;
    } else {
      DisplayDigitEnable[4] &= 255 - 12;
    }
  }
}
...
void RPU_SetLampState(int lampNum, byte s_lampState, byte s_lampDim, int s_lampFlashPeriod) {
  ...
  if (lampNum == RPUstrobeLamp) {
    if (RPUstrobeType <= 1) {
      s_lampState = RPUstrobeType; // Always off, or always on. No dimming, no strobing.
      s_lampDim = 0;
      s_lampFlashPeriod = 0;
    }
    else if (RPUstrobeType == 2) { // Xenon strobe lamps have on/off reversed. No dimming.
      s_lampState = !s_lampState;
      s_lampDim = 0;
    }
    else if (RPUstrobeType == 4 && s_lampFlashPeriod == 0) // Reverse on/off signal, unless it is flashing. If flashing with other lamps, the signal must flash together.
      s_lampState = !s_lampState; 
  }
...
// Dave's Think Tank - Setup for strobe lamp
void RPU_SetupStrobeLamp(int lamp, byte type) {
  RPUstrobeLamp = lamp;
  RPUstrobeType = type;
  RPU_SetLampState(lamp, 0); // Turn strobe off to start
}
